...
- **在有序数据集上**
```
(一个数组00XX,找数组中第一个/最后一个满足某个条件的位置 OO...OOXX....XX) 704&34(模板),35,69,367,540 = 374(APT); 702(倍增,API)
540(corner case太多了); 658(heap)
(找到条件，形成00XX) 山峰/rotate array（画图）:153,33，1095；
852（简单山峰）
```
- **在未排序数据集上**
```
(无法找到一个条件，形成 XXOO 的模型 但可以根据判断，保留下有解的那一半或者去掉无解的一半),162（连绵山峰）, 1091（162延伸）
```
- **在答案集上**
```
